...
**Spectral Classification** model classifies the hypercubes it recieves into one of two labels: *Organic* and *Non-Organic*. The goal for MobiSpectral is to generate hypercubes in the full functional range considered (400-1000 nm). To show the efficacy of the system, we developed some downstream applications which use the power of spectral signatures to output one of the two labels mentioned above.
...
There are some technical aspects of MobiSpectral which are not present in the paper, like how to get the spectral signatures of points in the hypercubes on Android.
